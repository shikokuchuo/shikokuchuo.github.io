...
- **First-class async by default**  e.g. async rather than just parallel map
...
`serial_config()` now accepts **vector arguments** to register multiple custom serialization configurations simultaneously. This enhancement simplifies working with diverse data types across distributed systems, for example with both Arrow Tables and Polars DataFrames.
...
For HPC environments, `daemons()` now supports initial synchronization exceeding 10 seconds. This enhancement is particularly valuable for large-scale distributed computing setups where network latency or resource provisioning may require longer connection times.
...
Developed in collaboration with Joe Cheng, it's the only event-driven solution that fully integrates with Shiny's interfaces, and also scales to complex apps with many users [Link to vignette example](https://mirai.r-lib.org/articles/v3-promises.html#shiny-extendedtask-introduction).

#### Powers Purrr

mirai's `mirai_map()` provides the underlying implementation for purrr's new `purrr::map(.parallel = TRUE)` and friends (currently in the development version). Purrr is one of the core tidyverse packages, and a mature functional programming tookit used by millions of data scientists and developers alike.
