...
Let us take a simple example, where we would like to calculate the ichimoku clouds for a portfolio of stocks, but also preserve the volume data, all in one tidy object.

### Functional programming setup

We could set it up as per the below:

- tickers: a vector defining the stock symbols in our portfolio
- process: a function that generates a row in a data frame / table
...
process <- function(x) {
  # Use getSymbols() from the 'quantmod' package to download pricing data
  pxdata <- quantmod::getSymbols(x, from = "2020-04-15", auto.assign = FALSE)
...
# Create output matrix by calling rbind on each elemnent of the list
...
Furthermore, 'i' and 'out' remain as leftover objects in the global environment. Somewhat messy.
...
- This can then be fed into do.call, which calls a function ('cbind') on a list of arguments (the output of lapply i.e. the lists returned by 'process').

The use of this do.call / lapply combination for similar tasks provides for a more succinct and distinctive coding style.
