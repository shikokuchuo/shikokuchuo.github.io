...
description: |
  Distill, Github Actions and RSelenium
author:
  - name: shikokuchuo
date: 05-03-2021
output:
...
This website is created using [Distill](https://pkgs.rstudio.com/distill/), a publication format for scientific and technical writing, native to the web.
...
Github actions allow you to set up a cron job to run your R scripts at specified times.
...
4. Assumes your repository branch is 'main', change if necessary.

5. Note that this script is run on a Windows VM using Windows R binary packages. This is currently much faster than building a lot of dependencies on Linux (which is also prone to failure).
</aside>
